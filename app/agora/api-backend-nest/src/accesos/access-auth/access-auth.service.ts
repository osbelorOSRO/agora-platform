...
export class AccessAuthService {
  ...
  async login(
    ...
    if (usuario.bloqueado)
      throw new ForbiddenException(
        'Cuenta bloqueada. Contacta al administrador del sistema.',
      );
    ...
    await this.prisma.sesion
      ...
      .catch((e: Error) =>
        this.logger.error(`Error registrando sesión: ${e.message}`),
      );
  ...
  async registrarUsuario(
    ...
    if (!usuario.invitation_token || !usuario.invitation_expires_at)
      throw new ForbiddenException(
        'Invitación no válida. Contacta al administrador.',
      );
    if (usuario.invitation_expires_at < new Date())
      throw new ForbiddenException(
        'La invitación ha expirado. Contacta al administrador.',
      );
    if (usuario.invitation_attempts >= MAX_INVITATION_ATTEMPTS)
      throw new ForbiddenException(
        'Invitación bloqueada por intentos fallidos. Contacta al administrador.',
      );

    const tokenOk = await bcrypt.compare(
      invitationToken,
      usuario.invitation_token,
    );
    if (!tokenOk) {
      await this.prisma.usuarios.update({
        where: { username },
  ...
  async resetPassword(
    ...
    if (usuario.reset_token_expires < new Date())
      throw new ForbiddenException(
        'El token de reset ha expirado. Contacta al administrador.',
      );
  ...
  async setup2FAInit(username: string, password: string, bypassToken: string) {
    ...
    if (
      !usuario.mfa_bypass_token ||
      !usuario.mfa_new_secret ||
      !usuario.mfa_reset_expires
    )
      throw new ForbiddenException('Token de reset 2FA no válido');
    if (usuario.mfa_reset_expires < new Date())
      throw new ForbiddenException(
        'El token de reset 2FA ha expirado. Contacta al administrador.',
      );
  ...
  async setup2FAConfirmar(
    ...
    if (!totpOk)
      throw new ForbiddenException(
        'Código TOTP incorrecto. Asegúrate de haber escaneado el código QR.',
      );

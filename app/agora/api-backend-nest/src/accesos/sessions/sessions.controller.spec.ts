...
const SUPERADMIN_PAYLOAD = { id: 12, username: 'obeltran', rol: 'superadmin', permisos: [] };
const AGENTE_PAYLOAD = { id: 19, username: 'fmartinez', rol: 'agente', permisos: [] };
...
describe('SessionsController', () => {
  ...
  describe('POST /api/auth/registrar-sesion', () => {
    it('returns 200 on session registration', async () => {
      ...
      const res = await request(app.getHttpServer()).post('/api/auth/registrar-sesion').expect(200);
      expect(res.body).toHaveProperty('sessionId');
      expect(mockService.registrarSesion).toHaveBeenCalledWith(12, expect.any(String), expect.any(String));
    });

    it('returns 401 when token is absent', async () => {
      app = await buildApp(unauthorizedGuard);
      await request(app.getHttpServer()).post('/api/auth/registrar-sesion').expect(401);
    });
  });
  ...
  describe('GET /api/auth/sesiones-activas', () => {
    it('returns 200 with active sessions list', async () => {
      ...
      const res = await request(app.getHttpServer()).get('/api/auth/sesiones-activas').expect(200);
      expect(Array.isArray(res.body)).toBe(true);
    });

    it('returns 401 when token is absent', async () => {
      app = await buildApp(unauthorizedGuard);
      await request(app.getHttpServer()).get('/api/auth/sesiones-activas').expect(401);
    });
  });
  ...
  describe('GET /api/auth/sesiones-activas-admin', () => {
    it('returns 200 for superadmin', async () => {
      ...
      await request(app.getHttpServer()).get('/api/auth/sesiones-activas-admin').expect(200);
    });

    it('returns 403 for agente rol', async () => {
      app = await buildApp(makeAuthGuard(AGENTE_PAYLOAD));
      await request(app.getHttpServer()).get('/api/auth/sesiones-activas-admin').expect(403);
    });

    it('returns 401 when token is absent', async () => {
      app = await buildApp(unauthorizedGuard);
      await request(app.getHttpServer()).get('/api/auth/sesiones-activas-admin').expect(401);
    });
  });
  ...
  describe('DELETE /api/auth/sesiones/:id', () => {
    it('returns 200 for superadmin closing a session', async () => {
      ...
      await request(app.getHttpServer()).delete('/api/auth/sesiones/5').expect(200);
      expect(mockService.cerrarSesionAdmin).toHaveBeenCalledWith(5);
    });

    it('returns 403 for agente rol', async () => {
      app = await buildApp(makeAuthGuard(AGENTE_PAYLOAD));
      await request(app.getHttpServer()).delete('/api/auth/sesiones/5').expect(403);
    });

    it('returns 400 when id is not a number', async () => {
      app = await buildApp(makeAuthGuard(SUPERADMIN_PAYLOAD));
      await request(app.getHttpServer()).delete('/api/auth/sesiones/abc').expect(400);
    });
  });
});

...
describe('RolesController', () => {
  ...
  describe('GET /api/roles', () => {
    it('returns 200 with roles list', async () => {
      ...
      mockService.obtenerRoles.mockResolvedValue([{ id: 1, nombre: 'superadmin' }]);
      const res = await request(app.getHttpServer()).get('/api/roles').expect(200);
  ...
  describe('GET /api/roles/:id', () => {
    it('returns 200 with the role', async () => {
      app = await buildApp(makeAuthGuard(WITH_PERM));
      mockService.obtenerRolPorId.mockResolvedValue({ id: 4, nombre: 'agente' });
      const res = await request(app.getHttpServer()).get('/api/roles/4').expect(200);
    ...
    it('returns 404 when role does not exist', async () => {
      app = await buildApp(makeAuthGuard(WITH_PERM));
      mockService.obtenerRolPorId.mockRejectedValue(new NotFoundException('Rol no encontrado'));
  ...
  describe('POST /api/roles', () => {
    ...
    it('returns 201 on successful creation', async () => {
      ...
      const res = await request(app.getHttpServer()).post('/api/roles').send(validBody).expect(201);
      expect(res.body).toHaveProperty('nombre', 'supervisor');
      expect(mockService.crearRol).toHaveBeenCalledWith('supervisor', [1, 2, 3], 12);
    });

    it('returns 400 when nombre is missing', async () => {
      app = await buildApp(makeAuthGuard(WITH_PERM));
      await request(app.getHttpServer()).post('/api/roles').send({ permisos: [1] }).expect(400);
    });

    it('returns 400 when permisos is not an array of integers', async () => {
      app = await buildApp(makeAuthGuard(WITH_PERM));
      await request(app.getHttpServer()).post('/api/roles').send({ nombre: 'test', permisos: ['a', 'b'] }).expect(400);
    });

    it('returns 403 when permission is missing', async () => {
      app = await buildApp(makeAuthGuard(WITHOUT_PERM));
      await request(app.getHttpServer()).post('/api/roles').send(validBody).expect(403);
    });
  });
  ...
  describe('PUT /api/roles/:id', () => {
    ...
    it('returns 200 on successful update', async () => {
      app = await buildApp(makeAuthGuard(WITH_PERM));
      mockService.actualizarRol.mockResolvedValue({ id: 5, nombre: 'supervisor-v2' });
      const res = await request(app.getHttpServer()).put('/api/roles/5').send(validBody).expect(200);
      expect(res.body).toHaveProperty('nombre', 'supervisor-v2');
      expect(mockService.actualizarRol).toHaveBeenCalledWith(5, 'supervisor-v2', [1, 4], 12);
    });

    it('returns 400 when body is missing required fields', async () => {
      app = await buildApp(makeAuthGuard(WITH_PERM));
      await request(app.getHttpServer()).put('/api/roles/5').send({}).expect(400);
    });

    it('returns 400 when id is not a number', async () => {
      app = await buildApp(makeAuthGuard(WITH_PERM));
      await request(app.getHttpServer()).put('/api/roles/abc').send(validBody).expect(400);
    });

    it('returns 403 when permission is missing', async () => {
      app = await buildApp(makeAuthGuard(WITHOUT_PERM));
      await request(app.getHttpServer()).put('/api/roles/5').send(validBody).expect(403);
    });
  });
});

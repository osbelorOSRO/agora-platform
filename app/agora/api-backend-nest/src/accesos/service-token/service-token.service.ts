...
import {
  IVaultGateway,
  VAULT_GATEWAY,
} from '../../auth/interfaces/vault-gateway.interface';
...
export class ServiceTokenService {
  ...
  async issueServiceToken(
    ...
    if (!serviceId || !secretKey)
      throw new BadRequestException('serviceId y secretKey son requeridos');
    if (
    ...
    try {
      storedSecret = await this.vaultService.getSecretKey(
        `accesos/service-keys/${serviceId}`,
      );
    } catch {

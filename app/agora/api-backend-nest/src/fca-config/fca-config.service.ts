...
export class FcaConfigService {
  ...
  constructor(
    ...
    const raw = process.env.FCA_CONFIG_ENCRYPTION_KEY;
    if (!raw) throw new Error('FCA_CONFIG_ENCRYPTION_KEY no está definido');
    this.encKey = scryptSync(raw, 'fca_config_salt', 32);
  }
  ...
  private encryptRow(dto: UpdateFcaConfigDto): Record<string, string | null> {
  ...
  async get(): Promise<Record<string, string | null>> {
    ...
    const rows = await this.prisma.$queryRawUnsafe<Record<string, unknown>[]>(
      `SELECT * FROM fca_app_config WHERE id = 1 LIMIT 1`,
    );
  ...
  async reveal(field: string): Promise<string | null> {
    if (!ENCRYPTED_FIELDS.has(field)) return null;
    return this.getSecret(field);
  }

  async upsert(
    dto: UpdateFcaConfigDto,
    ...
    await this.prisma.$queryRawUnsafe(
      `INSERT INTO fca_app_config (id, ${fields.join(', ')}, updated_at)
    ...
    this.logger.log(`fca_app_config actualizado: ${fields.join(', ')}`);
    return this.get();
  }

  async updateStatusFields(data: {
    fb_user_id?: string;
    fb_user_name?: string;
  }): Promise<void> {

...
export class MessagesProcessor extends WorkerHost {
  ...
  constructor(
    ...
    @InjectQueue(Q_THREAD_MSG_DELEGATION)
    private readonly threadDelegationQueue: Queue,
  ) {
    super();
  }
  ...
  private async processInner(job: Job<any>): Promise<void> {
    ...
    const messageDirection = this.normalizer.resolveDirection(
      env?.payload || {},
      eventKind,
    );
    ...
    await this.prisma.$transaction(async (tx) => {
      ...
      this.logger.log(
        `FLOW[MESSAGE] event_history ok externalEventId=${env.externalEventId}`,
      );

      await this.bootstrap.ensureActorExists(tx, env.actorExternalId);
      this.logger.log(
        `FLOW[MESSAGE] bootstrap ok actorExternalId=${env.actorExternalId}`,
      );
    });
    ...
    if (eventKind === 'message_echo') {
      ...
      this.logger.log(
        `FLOW[MESSAGE] done externalEventId=${env.externalEventId}`,
      );
      return;
    }

    let inboxPersistResult: {
      inserted: boolean;
      primedFirstDelegate: boolean;
    } | null = null;
    if (this.delegationGate.isInboxMessageEvent(env?.eventType)) {
      inboxPersistResult =
        await this.messagePersistence.persistMessageSession(env);
      this.logger.log(
        `FLOW[MESSAGE] inbox persisted actor=${env.actorExternalId}`,
      );
      if (!inboxPersistResult.inserted) {
        this.logger.warn(
          `FLOW[MESSAGE] duplicate inbox event stop externalEventId=${env.externalEventId}`,
        );
        return;
      }
    } else {
      this.logger.log(
        `FLOW[MESSAGE] inbox skip non-message eventType=${env?.eventType}`,
      );
    }

    if (
      !this.delegationGate.isDelegableIncomingEvent(
        env?.provider,
        env?.eventType,
        messageDirection,
      )
    ...
    await this.prisma.$transaction(async (tx) => {
      const delegationControl =
        await this.delegationGate.getDelegationControlState(
          tx,
          env.actorExternalId,
      ...
      if (delegationControl?.blocked) {
        ...
        if (
          delegationControl.reason === 'awaiting_first_incoming_delegate' &&
          delegationControl.sessionId
        ) {
          await this.delegationGate.clearAwaitingFirstIncomingDelegate(
            tx,
            delegationControl.sessionId,
          );
        }
      ...
      if (
        delegationControl?.sessionId &&
        (delegationControl.attentionMode ?? 'N8N') === 'N8N'
      ) {
        const threadDelegationPayload =
          await this.delegationGate.buildThreadDelegationPayload(
            tx,
            env,
            delegationControl,
          );
      ...
      const state = await this.delegationGate.getLatestLifecycleState(
        tx,
        env.actorExternalId,
      );
      ...
      if (state === 'QUALIFIED') {
        this.logger.log(
          `FLOW[MESSAGE] inbox branch actor=${env.actorExternalId} state=QUALIFIED`,
        );
        return;
      }
      ...
      if (state === 'BLOCKED') {
        this.logger.log(
          `FLOW[MESSAGE] blocked branch stop actor=${env.actorExternalId}`,
        );
        return;
      }
      ...
      if (isTerminal) {
        this.logger.log(
          `FLOW[MESSAGE] terminal gate stop actorExternalId=${env.actorExternalId}`,
        );
        return;
      }
      ...
      this.logger.log(
        `FLOW[MESSAGE] delegated externalEventId=${env.externalEventId}`,
      );
    });

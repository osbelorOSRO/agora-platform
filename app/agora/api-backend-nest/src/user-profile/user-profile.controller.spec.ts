...
const panelGuard = {
  canActivate: (ctx: any) => {
    ctx.switchToHttp().getRequest().userPayload = { id: 7, rol: 'agente', permisos: [] };
    return true;
  },
};
...
describe('UserProfileController', () => {
  ...
  describe('GET /user-profile/photo', () => {
    it('devuelve 200 con photoUrl cuando existe', async () => {
      ...
      mockService.getPhotoUrl.mockResolvedValue('https://cdn.example.com/photo.jpg');
      const res = await request(app.getHttpServer()).get('/user-profile/photo').expect(200);
      expect(res.body).toHaveProperty('photoUrl', 'https://cdn.example.com/photo.jpg');
  ...
  describe('POST /user-profile/photo', () => {
    it('devuelve 200 con nueva photoUrl al subir archivo', async () => {
      app = await buildApp(panelGuard);
      mockService.uploadPhoto.mockResolvedValue('https://cdn.example.com/new-photo.jpg');
      const res = await request(app.getHttpServer())
        .post('/user-profile/photo')
        .attach('photo', TINY_PNG, { filename: 'avatar.png', contentType: 'image/png' })
        .expect(201);
      expect(res.body).toHaveProperty('photoUrl', 'https://cdn.example.com/new-photo.jpg');
      expect(mockService.uploadPhoto).toHaveBeenCalledWith(7, expect.objectContaining({ originalname: 'avatar.png' }));
    });
  ...
  describe('DELETE /user-profile/photo', () => {
    it('devuelve 200 con ok:true al eliminar foto', async () => {
      ...
      const res = await request(app.getHttpServer()).delete('/user-profile/photo').expect(200);
    ...
    it('devuelve 401 sin token', async () => {
      app = await buildApp({ canActivate: () => { throw new UnauthorizedException(); } });
      await request(app.getHttpServer()).delete('/user-profile/photo').expect(401);
    });
  });
});

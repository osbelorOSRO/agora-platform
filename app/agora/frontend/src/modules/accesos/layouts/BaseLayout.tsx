...
export default function BaseLayout() {
  ...
  return (
    <div className="flex h-[100svh] w-full max-w-full overflow-hidden bg-background">
      <SidebarCompacto />
      <div className="flex flex-1 flex-col min-w-0 overflow-hidden">
        <header className="flex h-16 shrink-0 items-center justify-between border-b border-border glass-md px-4 md:px-8">
        ...
        <main
          className={`flex-1 min-h-0 bg-background overscroll-y-contain ${
            isImmersiveSection
              ? "flex flex-col overflow-hidden"
              : "overflow-y-auto px-4 pb-4 pt-6 md:px-6 md:pb-6"
          }`}
        >
          <Outlet />
        </main>

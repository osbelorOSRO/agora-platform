...
export class MinioService implements OnModuleInit {
  ...
  constructor() {
    ...
    this.publicBase = (process.env.MEDIA_BASE_URL || `http://${endpoint}:${port}`).replace(/\/+$/, '');
  ...
  async uploadFile(filePath: string, filename: string, mimeType: string): Promise<string> {
    ...
    await this.client.putObject(this.bucket, filename, stream, size, { 'Content-Type': mimeType });
